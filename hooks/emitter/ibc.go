...
import (
	...
	interchaincontrollertypes "github.com/cosmos/ibc-go/v7/modules/apps/27-interchain-accounts/controller/types"
...
func (ibca *IBCAdapter) CheckMsg(_ sdk.Context, msg sdk.Msg) {
	...
	case *feetypes.MsgPayPacketFeeAsync:
		ibca.isIBC = true
	case *interchaincontrollertypes.MsgRegisterInterchainAccount:
		ibca.isIBC = true
	case *interchaincontrollertypes.MsgSendTx:
		ibca.isIBC = true
...
func (ibca *IBCAdapter) HandleMsgEvents(_ sdk.Context, _ []byte, _ sdk.Msg, evMap common.EvMap, _ common.JsDict, _ *[]common.Message) {
	...
	if _, ok := evMap[ibcchanneltypes.EventTypeSendPacket+"."+ibcchanneltypes.AttributeKeyDataHex]; ok {
		ibca.isIBC = true
	}
	if _, ok := evMap[ibcchanneltypes.EventTypeWriteAck+"."+ibcchanneltypes.AttributeKeyDataHex]; ok {
		ibca.isIBC = true
	}
	if _, ok := evMap[ibcchanneltypes.EventTypeAcknowledgePacket+"."+ibcchanneltypes.AttributeKeyTimeoutHeight]; ok {
		ibca.isIBC = true
	}
	if _, ok := evMap[ibcchanneltypes.EventTypeTimeoutPacket+"."+ibcchanneltypes.AttributeKeyTimeoutHeight]; ok {
		ibca.isIBC = true
	}
}
