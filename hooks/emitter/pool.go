...
import (
	...
	"github.com/osmosis-labs/osmosis/v22/hooks/common"
	clpkeeper "github.com/osmosis-labs/osmosis/v22/x/concentrated-liquidity"
	concentratedpool "github.com/osmosis-labs/osmosis/v22/x/concentrated-liquidity/model"
	clptypes "github.com/osmosis-labs/osmosis/v22/x/concentrated-liquidity/types"
	cosmwasmpoolkeeper "github.com/osmosis-labs/osmosis/v22/x/cosmwasmpool"
	cosmwasmpool "github.com/osmosis-labs/osmosis/v22/x/cosmwasmpool/model"
...
func (pa *PoolAdapter) CheckMsg(ctx sdk.Context, msg sdk.Msg) {
	...
	case *clptypes.MsgTransferPositions:
		for _, positionId := range msg.PositionIds {
			if position, err := pa.clpKeeper.GetPosition(ctx, positionId); err == nil {
				pa.isClp = true
				pa.poolTxs[position.PoolId] = true
	...
	if rawIds, ok := evMap[govtypes.EventTypeActiveProposal+"."+govtypes.AttributeKeyProposalID]; ok {
		...
		for idx, rawId := range rawIds {
			...
			for _, proposalMsg := range proposalMsgs {
				switch proposalMsg := proposalMsg.(type) {
				case *govv1types.MsgExecLegacyContent:
					content, err := govv1types.LegacyContentFromMessage(proposalMsg)
					if err != nil {
						panic(err)
					}
					switch content := content.(type) {
					case *gammtypes.CreateConcentratedLiquidityPoolsAndLinktoCFMMProposal:
						numPoolsCreated := len(content.PoolRecordsWithCfmmLink)
					...
					case *gammtypes.SetScalingFactorControllerProposal:
						common.AppendMessage(kafka, "UPDATE_POOL", common.JsDict{
							"id":                        content.PoolId,
							"scaling_factor_controller": content.ControllerAddress,
						})
					...
					case *cosmwasmpooltypes.UploadCosmWasmPoolCodeAndWhiteListProposal:
						...
						codeId := pa.wasmKeeper.PeekAutoIncrementID(ctx, wasmtypes.KeySequenceCodeID) - 1
					...
					case *cosmwasmpooltypes.MigratePoolContractsProposal:
						...
						if len(content.WASMByteCode) > 0 {
							newCodeId = pa.wasmKeeper.PeekAutoIncrementID(ctx, wasmtypes.KeySequenceCodeID) - 1
						} else {
					...
					case *poolmanagertypes.DenomPairTakerFeeProposal:
						for _, denomPairTakerFee := range content.DenomPairTakerFee {
							denoms := []string{denomPairTakerFee.Denom0, denomPairTakerFee.Denom1}
							sort.Strings(denoms)
							common.AppendMessage(kafka, "SET_TAKER_FEE", common.JsDict{
								"denom0":    denoms[0],
								"denom1":    denoms[1],
								"taker_fee": denomPairTakerFee.TakerFee,
							})
						}
					}
				case *balancer.MsgCreateBalancerPool:
					poolId := pa.poolmanagerKeeper.GetNextPoolId(ctx) - uint64(1)
					pool, _ := pa.poolmanagerKeeper.GetPool(ctx, poolId)
					balancerPool, ok := pool.(*balancer.Pool)
					if !ok {
						panic(fmt.Errorf("cannot decode balancer pool from proposal, proposal => %d, pool id => %d", proposalId, poolId))
					}
					weights := make([]common.JsDict, 0)
					for _, weight := range balancerPool.PoolAssets {
						weights = append(weights, common.JsDict{
							"denom":  weight.Token.GetDenom(),
							"weight": weight.Weight.String(),
						})
					}
					govModuleAcc := pa.accountKeeper.GetModuleAccount(ctx, govtypes.ModuleName)
					poolCreatorAddress := govModuleAcc.GetAddress()
					poolMsg := common.JsDict{
						"id":                   poolId,
						"liquidity":            balancerPool.GetTotalPoolLiquidity(ctx),
						"type":                 balancerPool.GetType(),
						"creator":              poolCreatorAddress.String(),
						"is_superfluid":        false,
						"is_supported":         false,
						"swap_fee":             balancerPool.GetSpreadFactor(ctx).String(),
						"exit_fee":             balancerPool.GetExitFee(ctx).String(),
						"future_pool_governor": balancerPool.FuturePoolGovernor,
						"weight":               weights,
						"address":              balancerPool.GetAddress().String(),
						"total_shares":         balancerPool.TotalShares,
					}
					if smoothWeightChangeParams := balancerPool.PoolParams.GetSmoothWeightChangeParams(); smoothWeightChangeParams != nil {
						poolMsg["smooth_weight_change_params"] = smoothWeightChangeParams
					}
					common.AppendMessage(kafka, "NEW_OSMOSIS_POOL", poolMsg)
				case *stableswap.MsgCreateStableswapPool:
					poolId := pa.poolmanagerKeeper.GetNextPoolId(ctx) - uint64(1)
					pool, _ := pa.poolmanagerKeeper.GetPool(ctx, poolId)
					stableswapPool, ok := pool.(*stableswap.Pool)
					if !ok {
						panic(fmt.Errorf("cannot decode stableswap pool from proposal, proposal => %d, pool id => %d", proposalId, poolId))
					}
					govModuleAcc := pa.accountKeeper.GetModuleAccount(ctx, govtypes.ModuleName)
					poolCreatorAddress := govModuleAcc.GetAddress()
					common.AppendMessage(kafka, "NEW_OSMOSIS_POOL", common.JsDict{
						"id":                        poolId,
						"liquidity":                 stableswapPool.GetTotalPoolLiquidity(ctx),
						"type":                      stableswapPool.GetType(),
						"creator":                   poolCreatorAddress.String(),
						"is_superfluid":             false,
						"is_supported":              false,
						"swap_fee":                  stableswapPool.GetSpreadFactor(ctx).String(),
						"exit_fee":                  stableswapPool.GetExitFee(ctx).String(),
						"future_pool_governor":      stableswapPool.FuturePoolGovernor,
						"scaling_factors":           stableswapPool.GetScalingFactors(),
						"scaling_factor_controller": stableswapPool.ScalingFactorController,
						"address":                   stableswapPool.GetAddress().String(),
						"total_shares":              stableswapPool.TotalShares,
					})
				case *concentratedpool.MsgCreateConcentratedPool:
					poolId := pa.poolmanagerKeeper.GetNextPoolId(ctx) - uint64(1)
					pool, _ := pa.poolmanagerKeeper.GetPool(ctx, poolId)
					concentratedPool, ok := pool.(*concentratedpool.Pool)
					if !ok {
						panic(fmt.Errorf("cannot decode concentrated pool from proposal, proposal => %d, pool id => %d", proposalId, poolId))
					}
					poolmanagerModuleAcc := pa.accountKeeper.GetModuleAccount(ctx, poolmanagertypes.ModuleName)
				...
				case *cosmwasmpool.MsgCreateCosmWasmPool:
					poolId := pa.poolmanagerKeeper.GetNextPoolId(ctx) - uint64(1)
					pool, _ := pa.poolmanagerKeeper.GetPool(ctx, poolId)
					...
					govModuleAcc := pa.accountKeeper.GetModuleAccount(ctx, govtypes.ModuleName)
					poolCreatorAddress := govModuleAcc.GetAddress()
					pa.cosmwasmContractAddrs[cosmwasmPool.GetContractAddress()] = poolId
					common.AppendMessage(kafka, "NEW_OSMOSIS_POOL", common.JsDict{
						"id":                   poolId,
						...
						"is_superfluid":        false,
						"is_supported":         false,
						"swap_fee":             cosmwasmPool.GetSpreadFactor(ctx),
						"exit_fee":             "0",
						"future_pool_governor": "",
						"address":              cosmwasmPool.GetAddress(),
						"total_shares":         sdk.Coin{},
						"contract_address":     cosmwasmPool.GetContractAddress(),
					})
				}
			}
		}
...
// (from contracts, ica)
func (pa *PoolAdapter) handleNonPoolMsgsPoolActionEvents(ctx sdk.Context, evMap common.EvMap, kafka *[]common.Message) {
	...
	if takerFees, ok := evMap[poolmanagertypes.TypeMsgSetDenomPairTakerFee+"."+poolmanagertypes.AttributeKeyTakerFee]; ok {
		for idx, takerFee := range takerFees {
			denom0 := evMap[poolmanagertypes.TypeMsgSetDenomPairTakerFee+"."+poolmanagertypes.AttributeKeyDenom0][idx]
			denom1 := evMap[poolmanagertypes.TypeMsgSetDenomPairTakerFee+"."+poolmanagertypes.AttributeKeyDenom1][idx]
